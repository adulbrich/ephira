...
import {Accordion, DayData, DayDataStore, LoadData, MarkedDates, Mood, Symptoms, Values} from '@/constants/Interfaces'
...
export const useAccordion = create<Accordion>((set)=>({state: initialString,
  setExpandedAccordion: (data: string | null) => set(() => ({state: data}))
}
))

export const useMoods = create<Mood>((set)=>({selectedMoods: [],
  setSelectedMoods: (values: string[]) => set(() => ({selectedMoods: values}))
}
))

export const useSymptoms = create<Symptoms>((set)=>({selectedSymptoms: [],
  setSelectedSymptoms: (values: string[]) => set(() => ({selectedSymptoms: values}))
}
))

export const useValues = create<Values>((set)=>({selectedValues: [],
